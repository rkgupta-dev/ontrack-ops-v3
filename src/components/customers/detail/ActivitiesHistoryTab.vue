...
async function load() {
  ...
  error.value = null
  try {
    const result = await customersApi.fetchCustomerActivities({
...
<template>
  ...
  <EmptyState
    ...
    title="Couldn't load activities history"
    :message="toUserMessage(error)"
  >
    <v-btn class="mt-2" variant="tonal" color="primary" @click="load">Retry</v-btn>
  </EmptyState>

  <EmptyState v-else-if="rows.length === 0" icon="mdi-history" title="No activity found" />
      ...
      <v-table>
        <thead>
          <tr>
            ...
            <th class="font-weight-bold">For</th>
            <th class="font-weight-bold">Type</th>
            <th class="font-weight-bold">On</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in rows" :key="row.id">
            <td>{{ row.id }}</td>
            <td class="text-no-wrap">{{ row.description }}</td>
            <td>{{ row.type }}</td>
